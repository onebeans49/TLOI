...
# 로그 설정
logging.basicConfig(
    filename='20-1to40_log.log',    # 로그 파일 이름
    level=logging.INFO,     # 로그 레벨 설정 (INFO 수준 이상의 로그만 기록)
    format='%(asctime)s - %(levelname)s - %(message)s',     # 로그 포맷 설정
    datefmt='%Y-%m-%d %H:%M:%S'     # 로그 메시지의 날짜/시간 포맷 설정
)
...
def process_new_data():
    ...
    current_no = df_op21['no'].max()
    update_now = f"데이터 업데이트 확인중.. 현재 no: {current_no}.."
    print(update_now)

    if last_data is None or current_no != last_data:
        update_new = f"새로운 데이터 확인.. 현재 no: {current_no}.."
        print(update_new)
        last_data = current_no

        ''' 가져온 데이터 전처리 '''
        df_op21 = df_op21[['D12', 'D15', 'D21', 'D33']]
        df_op21.rename(columns={"D12": "OP20-1전장","D15": "OP20-1거리","D21": "OP20-1좌1차피크" ,"D33": "OP20-1우1차피크"}, inplace=True)

        x = df_op21[["OP20-1전장", "OP20-1거리", "OP20-1좌1차피크","OP20-1우1차피크"]]

        ''' 학습된 모델 불러온 후 예측하기 '''
        # 모델 불러오기
        loaded_model = joblib.load('model21.joblib')
        ...
        if prob < 0.9:
            ...
            data_dice = dice_ml.Data(dataframe=predicted_df,
                                     continuous_features=["OP20-1전장", "OP20-1거리", "OP20-1좌1차피크", "OP20-1우1차피크"],
        ...
        try:
            ...
            with conn.cursor() as cursor:
                # UPDATE SQL 쿼리 실행
                query1 = "UPDATE tbl_op20_1 SET passing_rate = %s WHERE no = %s;"
                query2 = "INSERT INTO tbl_op20_1_asset_defective_item (op20_1_no, item_name, item_value, item_solution) VALUES (%s, %s, %s, %s)"
                query3 = 'INSERT INTO tbl_op20_1_asset_defective_item (op20_1_no, item_name) VALUES (%s, %s);'
                ...
                if prob <= 0.5:
                    ...
                    cursor.execute(query3, (index + 1, item_name2))  # tbl_op20_1_defective_item에 삽입
